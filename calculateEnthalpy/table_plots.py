...
file_path = "./data/output_data/quinary_bcc_Cr_W_equiOut.csv"
...
df_proc = df.loc[df['Misc_T'] <= 0.95*df['Tm']]
df_proc = df_proc.sort_values(['Tm'])
plt.grid(axis = 'y', zorder = 0)
plt.bar(x = df_proc['Comp'], height = df_proc['Tm'], color = '#004488', zorder = 3, width = 0.2)
plt.bar(x = df_proc['Comp'], height = df_proc['Misc_T'], color = '#994455', zorder = 3, width = 0.2)
plt.xticks(rotation = 45)
plt.ylabel("Temperature (K)")

plt.xlabel("Composition")
plt.legend(["$Avg. T_{m}$", "$T_{misc}$"])
plt.show()
