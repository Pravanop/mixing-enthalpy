...
class calculateEnthalpyDataset:
	...
	def __init__(self,
		...
		self.tm = thermoMaths()
		# self.mpr = MPRester(
		#     api_key=getAPIKey("../callMpAPI/api_key.txt"), mute_progress_bars=True)
		...
		for dimension, pair_list in pairs.items():
			for idx, pair in enumerate(tqdm(pair_list, desc="Processing pairs created from element_list")):
				...
				if len(lattice_value_pair) > 0:
					result_dict[pair] = dict(
						zip(lattice_value_pair[:, 0], np.round(lattice_value_pair[:, 1].astype(float), 5)))
