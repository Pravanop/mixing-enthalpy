...
def extract_intermetallics():
	...
	mpr = MPRester(api_key = getAPIKey("/callMpAPI/api_key.txt"))
	fields = ['composition' , 'formation_energy_per_atom' , 'energy_above_hull' , 'chemsys']
	
	with open(
			"/calculateEnthalpy/data/output_data/old_data/dump_20240805-125555.json", 'r'
			) as f :
	...
	# load binary
	binary = dump_dict['2']  # taking default values for now. #BCC, bokas,
	# input_data/

	
	ternary_updated = find_n_nary_intermetallic(ternary , mpr , fields)
	binary_updated = find_n_nary_intermetallic(binary, mpr , fields)
	stop = time.time()
	print(stop - start)
