...
class MiscellaneousVisualizations:
	...
	def heatmap(self, element_list: List[str], lattice: Literal['FCC', 'BCC', 'HCP'], sns=None) -> plt.Figure:
		"""
		...
		for pair in binaries:
			...
			value = int(self.grid_iterator.data[pair][lattice] * 1000 / 4) if self.flags['correction'] else int(
				self.grid_iterator.data[pair][lattice] * 1000)
