...
class Visualizations:
	...
	def __init__(self,
		...
		self.grid_iterator = GridIterator(grid_size=self.grid_size,
										  tm=self.tm,
										  data=data,
	...
	def save_figure(self, folders: List[str], file_name: str, fig: plt.Figure):
		...
		if self.flags['equi_flag']:
			fig.savefig(
				fname=f"{updated_folder_path}{file_name}_equi.png",
				dpi=100
			)
		
		if not self.flags['im_flag']:
			fig.savefig(
				fname=f"{updated_folder_path}{file_name}_wo_im.png",
				dpi=100
			)
		
		if not self.flags['correction']:
			fig.savefig(
				fname=f"{updated_folder_path}{file_name}_wo_correction.png",
				dpi=100
			)
