...
def phase_diagram_visualizations(input_list, meta_data, lattice):
	...
	if n_alloy == 2:
		...
		plot_options = st.radio("Select the type of Phase Diagram", ['Miscible T contours', 'Isotherms'], index = None)
		if plot_options:
		...
		with st.spinner('Wait for it...'):
			pV = PolarVisualizations(composition=input_list, lattice=lattice, meta_data=meta_data, save_flag=False, type_flag = 'misc_T')
			with st.spinner('Wait for it...'):
