...
def composition_information(input_list, meta_data, lattice):
	sub_options = st.selectbox('What would you like to do?', options=['Miscible Temperature', 'Decomposition Products'])
	...
	for i in range(len(input_list)):
		with cols[i]:
			mol_inputs.append(
				st.number_input(f'Mole fraction of {input_list[i]}', min_value=0.0, max_value=1.0, value=0.0))
