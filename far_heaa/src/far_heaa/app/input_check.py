...
class InputCheck:
	...
	def __init__(self, input_str):
		...
		self.all_element_list = TextHandler.extract_ele_list(folder_path=self.folder_path, file_name='all_elements')
		self.all_element_set = set(self.all_element_list)
		self.database_element_list = TextHandler.extract_ele_list(folder_path=self.folder_path,
																  file_name='database_element_list')
		self.database_element_set = set(self.database_element_list)
	
	def subset_all(self):
		return self.input_set.issubset(self.all_element_set) and self.input_set != self.all_element_set
	
	def subset_database(self):
		return self.input_set.issubset(self.database_element_set) and self.input_set != self.database_element_set
	...
	def input_validation(self):
		...
		if not subset_all_flag:
			st.write(f"{', '.join(list(self.input_set.difference(self.all_element_set)))} not a valid element!")
			return invalid_flag
