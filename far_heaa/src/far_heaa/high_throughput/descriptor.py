...
for key, value in file.items():
    for add_ele, temp_list in value.items():
        ...
        for j in composition:
            ...
            im_formation += [k.energy / k.composition.num_atoms for k in im_list['-'.join(sorted([j, add_ele]))] if '-'.join(sorted([j, add_ele])) in im_list]
        ...
        if '-'.join(sorted(composition + [add_ele])) in im_list:
            im_formation += [k.energy / k.composition.num_atoms for k in im_list['-'.join(sorted(composition + [add_ele]))] ]
...
X = df[['Ha', 'Hc', 'delta meltT', 'im']].to_numpy()
