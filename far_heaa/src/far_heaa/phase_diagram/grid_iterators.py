...
class GridIterator:
	...
	def misc_temperature_across_grid(
        ...
        Args:
                is_differential (bool): If True, use (average melting temperature - miscibility_temperature) instead of miscibility temperature.
                composition (List[str]): A list of elements in the alloy.
			...
			for idx, mol in enumerate(mol_grid):
				...
				if is_stable is not None:
					...
					if np.isclose(is_stable[1], 0.0, atol=1e-3):
						if is_differential:
							avg_tm = self.tm.avg_T_melt(composition, list(mol))
							stables[idx] = avg_tm - temp
						else:
							stables[idx] = temp
						done_idx.append(idx)
					else:
