...
def POSCAR(inp_file_path , named_output_folder_path) :
	...
	species , species_count = get_element_symbols(s)
...
def runjob(metadata , named_output_folder_path , name) :
	...
	if runjob_meta["supercomputer"] == "expanse" :
		...
		if runjob_meta['vasp_type'] == '2D' :
			runjob_lines += f"mpirun $VASP2d >> stdout\n"
		if runjob_meta['vasp_type'] == 'std' :
			runjob_lines += f"mpirun $VASP >> stdout\n"
		if runjob_meta['vasp_type'] == 'z' :
			runjob_lines += f"mpirun $VASPz >> stdout\n"
		...
		if runjob_meta['vasp_type'] == '2D' :
			runjob_lines += f"ibrun $VASP2d >> stdout\n"
		if runjob_meta['vasp_type'] == 'std' :
			runjob_lines += f"ibrun $VASP >> stdout\n"
		if runjob_meta['vasp_type'] == 'z' :
			runjob_lines += f"ibrun $VASPz >> stdout\n"
		string_to_file(runjob_lines , runjob_file_path)
		string_to_file(runjob_lines , runjob_file_path)
