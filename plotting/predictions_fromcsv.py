...
lattice = 'BCC'
equi = False
df = pd.read_csv(f"./misc_T_{dim}_equi_{lattice}.csv")
df_proc = df.loc[df['Temp'] < 0.8*df['Avg_Tm']]
df_proc = df_proc[df['Alloys'].str.contains("W")]
...
if equi:
	plt.savefig(f'../plots/misc_T_{dim}_equi_{lattice}_W.png')
else:
	plt.savefig(f'../plots/misc_T_{dim}_{lattice}_W.png')
