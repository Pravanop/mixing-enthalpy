...
class MiscellaneousVisualizations:
	...
	def __init__(self, lattice: str, meta_data: dict, save_flag: bool):
		"""
	...
	def plot_convex_hull(self, composition: List[str], temperature: float) -> Tuple[plt.Axes, plt.Figure]:
		"""
		...
		if self.save_flag:
			...
			save_path = f"{updated_folder_path}/convex_hull_{temperature}_equi.png" if self.flags[
				'equi_flag'] else f"{updated_folder_path}/convex_hull_{temperature}.png"
			fig.savefig(fname=save_path, dpi=100)
		
		return ax, fig
	
	def heatmap(self, element_list: List[str], genre: str, sns=None) -> plt.Figure:
		"""
		...
		Args:
			...
			genre (str): The genre or type of mixing enthalpy to display (e.g., 'biased', 'unbiased').
		...
		for pair in binaries:
			...
			value = int(self.data[pair][genre] * 1000 / 4) if self.flags['correction'] else int(
				self.data[pair][genre] * 1000)
		...
		fig = sns.heatmap(alloy, cmap="coolwarm", annot=True, fmt='g', mask=alloy.isnull(), square=True,
						  cbar_kws={'label': 'Binary H$_{mix}$ (meV/atom)'}, linewidths=2, linecolor='white')
