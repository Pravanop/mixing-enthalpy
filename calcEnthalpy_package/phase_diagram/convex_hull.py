...
class ConvexHull:
	...
	def make_convex_hull(self, composition: List[str], temperature: float, batch_tag: bool = False,
						 **kwargs) -> PhaseDiagram:
		"""
				...
				for mol_frac in mol_grid:
					...
					if isinstance(mix_enthalpy, dict):
						for key, enthalpy in mix_enthalpy.items():
							...
							pd_entries_list.append(PDEntryLocal(
								composition=name,
								energy=self.tm.calc_gibbs_energy(enthalpy, config_entropy,
	...
	def update_convex_hull(self, composition: List[str], conv_hull: PhaseDiagram, temperature: float) -> PhaseDiagram:
		"""
		...
		for entry in pd_entries:
			...
			if 'MP' in entry.name:
				...
				if len(mol_ratio) > 1:
					...
					if isinstance(mix_enthalpy, dict):
						entry.update_energy(self.tm.calc_gibbs_energy(mix_enthalpy[lattice], config_entropy,
																	  temperature) * entry.composition.num_atoms)
						new_pd_entries.append(entry)
		
		new_pd_entries = self.process_elements_hull(composition, temperature, new_pd_entries)
		return PhaseDiagram(new_pd_entries)
	
	def process_elements_hull(self,
							  composition: List[str],
							  temperature: float,
							  pd_entries_list: List[PDEntryLocal]) -> List[PDEntryLocal]:
		"""
		...
		for ele in composition:
			if ele in ['Fe', 'Ti', 'Mn', 'Hf', 'Zr'] and self.correction:
				for key, value in self.end_member[ele].items():
					...
					temp_energy = value - value * temperature / self.transition_temperatures[ele][2] if key == \
																										self.transition_temperatures[
																											ele][
																											1] else value
	...
	def find_enthalpy_entropy_composition(self, composition: List[str], lattice: str, mol_ratio: List[float],
										  temperature: float) -> tuple[Union[int, float], float, dict[Any, Any]]:
		"""
		...
		if len(mol_ratio) == 1:
			...
			mix_enthalpy = min(list(mix_enthalpy.values())) if lattice == 'min' else mix_enthalpy[lattice]
