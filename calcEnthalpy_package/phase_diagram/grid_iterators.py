...
class GridIterator:
	...
	def upper_limit(self, composition: List[str], mol_ratio: List[float], phase_flag: bool) -> None:
		"""
	...
	def temp_iterator(self, composition: List[str], temp_grid: List[float]) -> Dict[float, PhaseDiagram]:
		"""
	...
	def misc_temperature_across_grid(self,
		...
		Args:
			composition (List[str]): A list of elements in the alloy.
			mol_grid_size (int): The size of the mole fraction grid. If not an integer, it is assumed to be the grid itself.
		...
		if 'conv_hull' in kwargs:
			...
			conv_hull_dict = self.temp_iterator(composition=composition, temp_grid=temp_grid)
	...
	def e_hull_across_grid(self,
						   ...
						   temp_gradation: int = 300) -> Tuple[ndarray, ndarray, ndarray]:
		"""
	...
	def find_decomp_products(self,
							 ...
							 lattice: str,
							 batch_tag: bool = False,
							 **kwargs) -> Union[tuple[DataFrame, float], tuple[None, int]]:
		"""
